      ...
      <TabsList className="w-full p-0 bg-background justify-start border-b rounded-none">
        {tabs.map((tab) => (
          <TabsTrigger
            key={tab.value}
            value={tab.value}
            className="rounded-none bg-background h-full data-[state=active]:shadow-none !border-x-transparent !border-t-transparent data-[state=active]:border-b-primary dark:data-[state=active]:border-b-primary"
          >
            <code className="text-[13px]">{tab.name}</code>
          </TabsTrigger>
        ))}
...
export function IconNavigationTabs() {
  return (
    <Tabs defaultValue={tabs[0].value} className="max-w-xs w-full">
      <TabsList className="h-14 w-full grid grid-cols-4 p-0 bg-background justify-start rounded-none">
        {tabs.map((tab) => (
          <TabsTrigger
            key={tab.value}
            value={tab.value}
            className="flex flex-col rounded-none bg-background h-full data-[state=active]:shadow-none !border-b-2 !border-transparent data-[state=active]:!border-b-primary [&>svg]:h-5 [&>svg]:w-5 [&>svg]:shrink-0"
          >
            {tab.icon}
